...
def expected_uuids() -> tuple[str | None, str | None]:
    data_uuid, config_uuid, errors = _load_expected_uuids()
    assert errors == []
    assert data_uuid is not None
    assert config_uuid is not None
    return data_uuid, config_uuid
...
def test_generated_files_validate(
    path: Path, expected_uuids: tuple[str | None, str | None]
) -> None:
    data_uuid, config_uuid = expected_uuids
