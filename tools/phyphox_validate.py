...
def validate_phyphox(path: str) -> list[ValidationError]:
    ...
    if _local_name(root.tag) != "phyphox":
        errors.append(ValidationError(f"{path}: root element must be <phyphox> (got <{_local_name(root.tag)}>)"))
        return errors
    ...
    if containers_el is not None:
        for c in _children(containers_el, "container"):
            name = _text(c)
            if not name:
                errors.append(ValidationError(f"{path}: <data-containers><container> must have non-empty text"))
                continue
    ...
    if len(container_set) != len(container_names):
        ...
        for n in container_names:
            if n in seen:
                dups.add(n)
            seen.add(n)
        errors.append(ValidationError(f"{path}: duplicate <container> names: {', '.join(sorted(dups))}"))

    # Collect container references from known phyphox structures only; extend if the
    # phyphox schema adds new elements that reference containers by name.
    referenced: set[str] = set()

    # <input><bluetooth> <output>CONTAINER</output>
    ...
    # <views> ... <graph> <input>CONTAINER</input> ... <value><input>CONTAINER</input>
    ...
    # <analysis> ... <input>CONTAINER</input> <output>CONTAINER</output>
    ...
    # <export><set><data>CONTAINER</data>
    ...
    if unknown:
        errors.append(ValidationError(f"{path}: references unknown data containers: {', '.join(unknown)}"))

    # Bluetooth plausibility: ensure input/output bluetooth IDs match; ensure the expected
    # cddf1002/1003 characteristics are used consistently.
    if input_el is not None:
        bt_inputs = _children(input_el, "bluetooth")
        if len(bt_inputs) != 1:
            errors.append(
                ValidationError(f"{path}: expected exactly one <input><bluetooth> block (found {len(bt_inputs)})")
            )
        else:
            bt = bt_inputs[0]
                ...
                for o in outs:
                    ...
                    if off is not None:
                        try:
                            ...
                            errors.append(ValidationError(f"{path}: invalid bluetooth output offset: {off!r}"))

                if not has_extra_time:
                    errors.append(ValidationError(f"{path}: missing bluetooth <output extra=\"time\"> mapping"))
                if len(data_chars) != 1:
                    errors.append(ValidationError(f"{path}: expected exactly one data characteristic UUID in inputs"))
                if offsets and offsets != {0, 4, 8, 12, 16}:
                    errors.append(
                        ValidationError(
                            f"{path}: expected float32 offsets {{0,4,8,12,16}} (got {sorted(offsets)})"
                        )
                    )

            out_el = _child(root, "output")
            if out_el is None:
                errors.append(ValidationError(f"{path}: missing <output> (used to push config to device)"))
            else:
                bt_outs = _children(out_el, "bluetooth")
                if len(bt_outs) != 1:
                    errors.append(
                        ValidationError(f"{path}: expected exactly one <output><bluetooth> block (found {len(bt_outs)})")
                    )
                else:
                    bt_out = bt_outs[0]
                    if bt_id and bt_out.attrib.get("id") != bt_id:
                        errors.append(ValidationError(f"{path}: bluetooth id mismatch between <input> and <output>"))

                    configs = _children(bt_out, "config")
                    if len(configs) != 1:
                        errors.append(
                            ValidationError(f"{path}: expected exactly one <output><bluetooth><config> (found {len(configs)})")
                        )
                    else:
                        cfg = configs[0]
                        if cfg.attrib.get("conversion") != "float32LittleEndian":
                            errors.append(
                                ValidationError(
                                    f"{path}: expected config conversion float32LittleEndian (got {cfg.attrib.get('conversion')!r})"
                                )
                            )
                        if not cfg.attrib.get("char"):
                        ...
                        if v is None:
                            ...
                            try:
                                ...
                                errors.append(ValidationError(f"{path}: <config> value is not numeric: {v!r}"))
...
def main() -> int:
    ...
    ap.add_argument("file", help="Path to *.phyphox file")
    args = ap.parse_args()

    errors = validate_phyphox(args.file)
    if errors:
        for e in errors:
